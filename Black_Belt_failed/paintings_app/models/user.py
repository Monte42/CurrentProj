...
from paintings_app.models import painting, purch
...
class User:
    ...
    def get_user_by_id(cls,id):
        ...
        join paintings
        on paintings.id = purchases.painting_id
        join users as users2
        on paintings.user_id = users2.id
        where users.id = %(id)s;
        '''
        ...
        if results:
            ...
            for row in results:
                ...
                this_purchase = purch.Purchase(purchase_data)
                this_purchase.user = cls(painters_data)
                this_purchase.painting = painting.Painting(painting_data)
                this_user.purchased_paintings.append(this_purchase)
            return this_user
        return False
